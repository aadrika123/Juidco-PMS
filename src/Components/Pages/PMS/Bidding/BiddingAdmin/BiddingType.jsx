...
const BiddingType = () => {
      ...
      .then(function (response) {
        if (response?.data?.status) {
          ...
          if (
            ...
            response?.data?.data?.techComparison &&
            response?.data?.data?.finCriteria?.length &&
            response?.data?.data?.bidder_master?.some(
              (data) => !data?.bidding_amount
            )
          ) {
            navigate("/bidding-commparision-tabs?tabNo=2", { state });
          } else if (
            response?.data?.data?.bid_type === "abc" &&
            response?.data?.data?.techComparison &&
            !response?.data?.data?.finComparison
          ) {
            navigate(`/finance-bidding/${state}`);
          }
        } else {
  ...
  const bidType =
    criteriaData?.bid_type === "financial"
      ...
      ? "finCriteria"
      : criteriaData?.bid_type === "abc" && !criteriaData?.techComparison
      ? "techCriteria"
      : criteriaData?.bid_type === "abc" &&
        criteriaData?.techComparison &&
        !criteriaData?.finComparison
      ? "finCriteria"
      : [];
      ...
      <div className={` ${isLoading ? "blur-[2px] pointer-events-none" : ""}`}>
        <div className={`px-6`}>
          <div className='flex justify-between gap-4'>
            ...
            <button
              className={`${
                criteriaData?.boq?.pre_tendering_details?.is_rate_contract
                  ? "bg-[#4338ca] text-white"
                  : "bg-gray-200 text-[#4338ca]"
              }  py-2 px-8 rounded-md`}
            >
          ...
          <div className=''>
            <div className='flex m-6'>
              <div className='bg-white w-[25rem] '>
                ...
                {criteriaData?.[bidType]?.map((data, index) => (
                  <div
                    className='pl-8 pt-5 pb-4 border border-gray-00'
                    key={index}
                  >
