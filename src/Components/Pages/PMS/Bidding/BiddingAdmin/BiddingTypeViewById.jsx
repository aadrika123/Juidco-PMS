...
const BiddingTypeViewById = () => {
  ...
  const selectWinnerPost = async () => {
    if (
      ...
      if (
        selectedBidder?.length > applicationData?.boq?.pre_tendering_details?.max_supplier &&
        applicationData?.boq?.pre_tendering_details?.is_rate_contract === true
      ) {
        return toast.error(
          `Selected Winners cannot be greater than the maximum no of Suppliers : ${applicationData?.boq?.pre_tendering_details?.max_supplier}`
        );
      } else if (
        selectedBidder?.length < applicationData?.boq?.pre_tendering_details?.min_supplier &&
        applicationData?.boq?.pre_tendering_details?.is_rate_contract === true
      ) {
        return toast.error(
          `Selected Winners cannot be less than the minimum no of Suppliers : ${applicationData?.boq?.pre_tendering_details?.min_supplier}`
        ); }
